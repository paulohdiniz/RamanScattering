...
%
SP = SP.pick_fourier_window('...'); %blackman, tukeywin, hamming, hann, flattopwin, '...' for nothing
...
%% TEST 
    ...
    subplot(2,3,3);
    % imagesc(squeeze(mean(abs(SP.data_stitched.data_R.^2),3)));
    ...
    figure ('Name', 'Raman Spectrum with mask selected','Position',[1000 100 600 400]),
    plot(SP.wn,SP.ramanSpectrumWithMask);
    xlabel('Wavenumbers [cm^{-1}]','fontsize',14);
    ylabel('Raman Spectrum','fontsize',14);
    xlim([0,160])
