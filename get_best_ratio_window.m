function best_ratio = get_best_ratio_window(SPtemp)
...
%analyse ratiowindow
    ...
    for i = 1:numel(ratios)
        ...
        SP(i) = SP(i).window_overlap_for_test(SP(i).tukey_window_param,SP(i).deadtime);
        SP(i) = SP(i).Tnorm_and_center_data(1,0,SP(i).deadtime);
        SP(i) = SP(i).stitch_time_axis_T_with_interp('makima'); %makima, pchirp, linear
        SP(i) = SP(i).pick_fourier_window([SP(i).window2_name]); %blackman, tukeywin, hamming, hann, flattopwin, '...' for nothing
    ...
    fprintf('The best ratio window is %s.\n',SP(best_indice).ratio_window)
    best_ratio = SP(best_indice).ratio_window;
